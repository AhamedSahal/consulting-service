...
const REDIRECT_URI =
  ...
  'http://localhost:5000/connect/onedrive/callback';
...
async function getAccessToken(companyId) {
  ...
  const c = conn.rows[0];
  const expiresAt = c.token_expires_at
    ? new Date(c.token_expires_at)
    : new Date(0);
  if (expiresAt.getTime() - Date.now() < 5 * 60 * 1000 && c.refresh_token) {
    ...
    const newExpires = result.expiresOn
      ? new Date(result.expiresOn)
      : new Date(Date.now() + 3600 * 1000);
    await pool.query(
...
async function uploadFile(companyId, buffer, filename, folderPath) {
  ...
  const basePath = (folderPath || 'HR Consulting AI/Exports').replace(
    /^\/+|\/+$/g,
    '',
  );
