...
function buildPdfBuffer(jdJson) {
  return new Promise((resolve, reject) => {
    ...
    if (j.job_family || j.department) {
      doc.text(`Department: ${j.job_family || j.department}`);
    }
    doc.moveDown(1);
    ...
    if (j.kpis && j.kpis.length) {
      doc.fontSize(12).text('Key Performance Indicators', {
        underline: true,
      });
      doc.moveDown(0.5);
