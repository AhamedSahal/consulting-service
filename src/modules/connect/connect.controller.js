...
const onedriveService = require('../../utils/onedriveService');
  ...
  if (!code) {
    return res.redirect(
      (process.env.FRONTEND_URL || 'http://localhost:3000') +
        '/connect?error=no_code',
    );
  }
  try {
    const cid =
      companyId ||
      (await pool.query('SELECT id FROM companies LIMIT 1')).rows[0]?.id;
    if (!companyId) {
      return res.redirect(
        (process.env.FRONTEND_URL || 'http://localhost:3000') +
          '/connect?error=no_company',
      );
    }
